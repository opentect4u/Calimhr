...
//For User Maintenance.......................................
	...
	public function addUserProcess(){
		...
		if(!$res){
			...
			if(!$res){
				...
				$res = $this->AdminProcess->addUserProcess($emp_no,$user_type,$user_id,$password,$status);
				if($res){
					$in = 1;
		    			redirect('Admin/userMaintenance/'.$in);
				}
	...
	public function editUserProcess(){
		...
		$eno  = $this->input->post('empno');

		$temp = $this->AdminProcess->delRow($emp_no);
...
//Purpose Wise Expence........................................................
	...
	/**************************Purposewise Expense Report InExcel************************/
		public function pwExpence_xlsx() {
			...
			$table_column = array("Date","Employee No.","Name","Purpose","Amount");
			...
			foreach($xldata as $row){
				...
				$object->getActiveSheet()->SetCellValueByColumnAndRow(0,$rowCount,$row->approval_dt);
				$object->getActiveSheet()->SetCellValueByColumnAndRow(1,$rowCount,$row->emp_no);
				$object->getActiveSheet()->SetCellValueByColumnAndRow(2,$rowCount,$row->emp_name);
				$object->getActiveSheet()->SetCellValueByColumnAndRow(3,$rowCount,$row->purpose);
				$object->getActiveSheet()->SetCellValueByColumnAndRow(4,$rowCount,$row->amount);

				$rowCount++;
			}

			$filename = "Purposewise_Expense-".date("d-m-Y H-i-s").'.xlsx';
 ...
 //For Individual claim Details...........................................       
  ...
  //For Employee Balance.........................
	...
	public function empBalance(){
	  if($this->session->userdata('is_login')->user_type == 'A' || $this->session->userdata('is_login')->user_type == 'AC' || $this->session->userdata('is_login')->user_type == 'M' || $this->session->userdata('is_login')->user_type == 'AU'){
...
// For Employee wise Personal Ledger

    public function adminLedgerDetails(){
        $title['title'] = 'Emp wise Ledger-Details';
		$t_name = 'mm_employee';
		$date1_temp = DateTime::createFromFormat('d/m/Y',$_POST['date1']);
		...
		$data['alldata'] = $this->AdminProcess->personalLedger($from_date,$to_date, $emp_no);
		$data['opening_balance'] = $this->AdminProcess->opening_balance($from_date, $emp_no);
		$data['emp_dtls'] = $this->AdminProcess->getDetailsbyEmpNo($t_name, $emp_no);
		$data['date'] = $this->AdminProcess->get_dt();
    	...
    	$title['total_reject'] = $this->Process->countRejClaim('tm_claim');
		$this->load->view('templetes/welcome_header',$title);
		$this->load->view('SU/adminLedgerDetails', $data);
		$this->load->view('templetes/welcome_footer');
    }
    
    public function empWisePersonalLedger(){
        if($this->session->userdata('is_login')->user_type == 'AC') {
           $result['dtls'] = $this->AdminProcess->getAll('mm_employee'); 
